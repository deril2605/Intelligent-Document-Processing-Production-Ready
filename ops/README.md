...
- `ops/analyzers/<document_type>/`
  - `create_<document_type>_analyzer.py`: direct, runnable analyzer creation script
  - `schema.json`: optional reference schema for that document type
...
2. Run that analyzer's creation script (single source of truth for deployment).
