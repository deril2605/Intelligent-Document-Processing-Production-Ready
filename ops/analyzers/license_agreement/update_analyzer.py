...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Update an existing license agreement ACU analyzer.")
    parser.add_argument("--analyzer-id", default=os.getenv("ACU_ANALYZER_ID"), help="ACU analyzer id to update")
    ...
    # ACU create API is upsert-style for analyzer definitions.
    response = client.begin_create_analyzer(
        analyzer_id=args.analyzer_id,
        analyzer_template=analyzer_schema,
    )

    print(f"Analyzer update request accepted for '{args.analyzer_id}'. status={response.status_code}")
    if args.wait:
